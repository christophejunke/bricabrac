(in-package :bricabrac.sdl2.sprites.transform)
...
(defun scale (sx &optional (sy sx))
  (check-type sx number)
  (check-type sy number)
  (make-transform sx sy 0 0))

(defun move (tx &optional (ty tx))
  (check-type tx number)
  (check-type ty number)
...
(defun scale-size (scale size)
  (typecase size
    (cons (destructuring-bind (unit value) size
            (ecase unit (:px value))))
    (number (* scale size))))
...
(defun pixel-rectangle (x y w h transform)
  (multiple-value-bind (px py) (transform x y transform)
    (with-transform (sx sy) transform
      (let ((unit-x (scale-size sx w))
            (unit-y (scale-size sy h)))
        (values
